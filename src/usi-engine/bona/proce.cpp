...
{
  if (isdigit((int)str[0]) && isdigit((int)str[1]) && isdigit((int)str[2]) && isdigit((int)str[3]) && isupper((int)str[4]) && isupper((int)str[5]) && str[6] == '\0')
  {
    return 1;
  }
...
static int CONV proce_cui(tree_t *restrict ptree)
{
  ...
  if (is_move(token))
  {
    return cmd_usrmove(ptree, token, &last);
  }
  if (!strcmp(token, "s"))
  {
    return cmd_move_now();
  }
  if (!strcmp(token, "beep"))
  {
    return cmd_beep(&last);
  }
  if (!strcmp(token, "book"))
  {
    return CmdBook(ptree, &last);
  }
  if (!strcmp(token, "display"))
  {
    return cmd_display(ptree, &last);
  }
  if (!strcmp(token, "hash"))
  {
    return cmd_hash(&last);
  }
  if (!strcmp(token, "limit"))
  {
    return cmd_limit(&last);
  }
  if (!strcmp(token, "move"))
  {
    return cmd_move(ptree, &last);
  }
  if (!strcmp(token, "new"))
  {
    return cmd_new(ptree, &last);
  }
  if (!strcmp(token, "outmove"))
  {
    return cmd_outmove(ptree);
  }
  if (!strcmp(token, "peek"))
  {
    return cmd_peek(&last);
  }
  if (!strcmp(token, "stdout"))
  {
    return cmd_stdout(&last);
  }
  ...
  {
    return cmd_ponder(&last);
  }
  if (!strcmp(token, "problem"))
  {
    return cmd_problem(ptree, &last);
  }
  ...
  {
    return cmd_read(ptree, &last);
  }
  if (!strcmp(token, "resign"))
  {
    return cmd_resign(ptree, &last);
  }
  ...
  if (!strcmp(token, "newlog"))
  {
    return cmd_newlog(&last);
  }
#if defined(CSA_LAN)
  if (!strcmp(token, "connect"))
  {
    return cmd_connect(ptree, &last);
  }
  if (!strcmp(token, "sendpv"))
  {
    return cmd_sendpv(&last);
  }
#endif
#if defined(MNJ_LAN)
  if (!strcmp(token, "mnj"))
...
#endif
#if defined(DFPN)
  if (!strcmp(token, "dfpn"))
  {
    return cmd_dfpn(ptree, &last);
  }
#endif
#if defined(DFPN_CLIENT)
  ...
  if (!strcmp(token, "tlp"))
  {
    return cmd_thread(&last);
  }
#endif
#if !defined(NO_STDOUT)
  if (!strcmp(token, "stress"))
  {
    return cmd_stress(&last);
  }
#endif
#if !defined(MINIMUM)
  if (!strcmp(token, "learn"))
  {
    return cmd_learn(ptree, &last);
  }
#endif
...
static int CONV proce_csalan(tree_t *restrict ptree)
{
  ...
  if (!strcmp(token, str_resign))
  {
    return cmd_resign(ptree, &last);
  }
  if (!strcmp(token, "#WIN") || !strcmp(token, "#LOSE") || !strcmp(token, "#DRAW") || !strcmp(token, "#CHUDAN"))
  {
    ...
    if (client_ngame == client_max_game)
    {
      return cmd_quit();
    }
...
static int CONV proce_mnj(tree_t *restrict ptree)
{
  ...
  {
    return cmd_mnjignore(ptree, &last);
  }
  if (!strcmp(token, "idle"))
  {
    return cmd_suspend();
  }
  if (!strcmp(token, "alter"))
  {
    return cmd_mnjmove(ptree, &last, 1);
  }
  ...
  if (!strcmp(token, "move"))
  {
    return cmd_mnjmove(ptree, &last, 0);
  }
...
cmd_mnjignore(tree_t *restrict ptree, char **lasts)
{
  ...
  for (i = 0;; i += 1)
  {
    ...
    if (interpret_CSA_move(ptree, &move, token) < 0)
    {
      return -1;
    }
...
usi_ignore(tree_t *restrict ptree, char **lasts)
{
  ...
  for (i = 0;; i += 1)
  {
    ...
    if (usi2csa(ptree, token, str_buf) < 0)
    {
      return -1;
    }
    if (interpret_CSA_move(ptree, &move, str_buf) < 0)
    {
      return -1;
    }
...
usi_go(tree_t *restrict ptree, char **lasts)
{
  ...
  char *ptr;
  int iret;
  long l,btime=0,wtime=0,binc=0,winc=0,byoyomi=0;
...
//	  printf("info string token %s\n", token);
	  ...
	  if (!strcmp(token, "infinite")) {
		  ...
		  if (ptr == token || (unsigned long)l > UINT_MAX || l < 0) break;
		  btime = l;
	  }
...
//  printf("info string btime %d wtime %d binc %d winc %d byoyomi %d\n", btime, wtime, binc, winc, byoyomi);
  ...
  if (root_turn) {
	  ...
	  remaintime = btime + (binc + byoyomi - minimum)*mtg -1000;
  }
  if (remaintime < 0) remaintime = 0;
  byoyomi = minimum+remaintime/mtg;
  if (ptree->nrep <= 30) byoyomi = (byoyomi * 7) / 10;
  if (byoyomi > 0) usi_byoyomi = byoyomi;
...
usi_posi(tree_t *restrict ptree, char **lasts)
{
  ...
  moves_ignore[0] = MOVE_NA;
  int line, i, j, nari, pc, num, hand, pos;
  size_t len;
  min_posi_t ini_posi;
...
//          printf(" * ");
	...
	ini_posi.turn_to_move = 0;
    token = strtok_r(NULL, str_delimiters, lasts);
    if (token[0] == 'w')
		ini_posi.turn_to_move = 1;

//    memset(motiw, 0, 128);
//    memset(motib, 0, 128);

	ini_posi.hand_black =0;
	ini_posi.hand_white =0;
	token = strtok_r(NULL, str_delimiters, lasts);
    len = strlen(token);
    num = 1;
    for (i = 0; i < len; i++)
    {
      ...
      {
      case 'P':
		  ini_posi.hand_black += num;
		  num = 1;
        break;
      case 'L':
		  ini_posi.hand_black += num<<5;
		  num = 1;
		  break;
      case 'N':
		  ini_posi.hand_black += num<<8;
		  num = 1;
		  break;
      case 'S':
		  ini_posi.hand_black += num<<11;
		  num = 1;
		  break;
      case 'G':
		  ini_posi.hand_black += num<<14;
		  num = 1;
		  break;
      case 'B':
		  ini_posi.hand_black += num<<17;
		  num = 1;
		  break;
      case 'R':
		  ini_posi.hand_black += num<<19;
		  num = 1;
		  break;
	  case 'p':
		  ini_posi.hand_white += num;
		  num = 1;
		  break;
	  case 'l':
		  ini_posi.hand_white += num << 5;
		  num = 1;
		  break;
	  case 'n':
		  ini_posi.hand_white += num << 8;
		  num = 1;
		  break;
	  case 's':
		  ini_posi.hand_white += num << 11;
		  num = 1;
		  break;
	  case 'g':
		  ini_posi.hand_white += num << 14;
		  num = 1;
		  break;
	  case 'b':
		  ini_posi.hand_white += num << 17;
		  num = 1;
		  break;
	  case 'r':
		  ini_posi.hand_white += num << 19;
		  num = 1;
		  break;
	  case '1':
	  case '2':
  ...
  ini_posi = min_posi_no_handicap;
}

  if (ini_game(ptree, &ini_posi,
  ...
  if (get_elapsed(&time_turn_start) < 0)
  {
    return -1;
  }
  return 1;
...
cmd_usrmove(tree_t *restrict ptree, const char *str_move, char **lasts)
{
  ...
  if (game_status & (flag_pondering | flag_puzzling))
  {
    ...
    if ((game_status & flag_puzzling) || strcmp(str_move, str_CSA_move(ponder_move)))
    {
  ...
  move_evasion_pchk = 0;
  iret = make_move_root(ptree, move, (flag_rep | flag_history | flag_time | flag_detect_hang));
  if (iret < 0)
  {
...
#endif
{
  ...
  else if (!strcmp(str, "narrow"))
  {
    game_status |= flag_narrow_book;
  }
...
static int CONV cmd_limit(char **lasts)
{
  ...
  else if (!strcmp(str, "time"))
  {
    ...
    else
    {
      ...
      {
        sec_limit_depth = (unsigned int)l3;
      }
    }
  }
  else
...
static int CONV cmd_move(tree_t *restrict ptree, char **lasts)
{
  ...
  if (str != ptr && l != LONG_MAX && l >= 1 && *ptr == '\0')
  {
    for (i = 0; i < l; i += 1)
    {
      if (game_status & (flag_move_now | mask_game_end))
      {
        break;
      }
  ...
  do
  {
    ...
    iret = make_move_root(ptree, move,
                          (flag_history | flag_time | flag_rep | flag_detect_hang));
    if (iret < 0)
    {
      return iret;
    }
...
static int CONV cmd_dfpn_client(tree_t *restrict ptree, char **lasts)
{
  ...
  if (ptr == str || dfpn_client_port == LONG_MAX || dfpn_client_port < 0 || dfpn_client_port > USHRT_MAX)
  {
    str_error = str_bad_cmdline;
    return -2;
  }
...
static int CONV cmd_connect(tree_t *restrict ptree, char **lasts)
{
  ...
  if (!str || !strcmp(str, "."))
  {
    str = "gserver.computer-shogi.org";
  }
  ...
  if (ptr == str || client_port == LONG_MAX || client_port < 0 || client_port > USHRT_MAX)
  {
    str_error = str_bad_cmdline;
    return -2;
  }
...
static int CONV cmd_sendpv(char **lasts)
{
  ...
  if (!strcmp(str, str_off))
  {
    game_status &= ~flag_sendpv;
  }
  else if (!strcmp(str, str_on))
  {
    game_status |= flag_sendpv;
  }
  else
...
static int CONV cmd_mnj(char **lasts)
{
  ...
  if (l <= 0)
  {
    mnj_depth_stable = INT_MAX;
  }
  else
  {
    mnj_depth_stable = (int)l;
  }
